...
class QuestService:
        ...
        if quest_data.google_calendar:
            try:
                ...
                if event_id:
                    ...
                    logger.info(
                        f"Created Google Calendar event {event_id} for quest {quest.id}"
                    )
    ...
    def update_quest(
        self, user_id: int, quest_id: int, update_data: schemas.QuestUpdate
    ) -> Optional[Quest]:
    ...
    def _calculate_quest_exp_reward(
        ...
        calculated_xp = int(
            base_xp * rarity_multipliers[rarity] * priority_multiplier * level_factor
        )
        ...
        is_exceptional = (
            ...
            and quest_type == QuestType.BOSS
            and capped_priority >= 4
        )
