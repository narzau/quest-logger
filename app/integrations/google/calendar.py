        ...
        try:
            ...
            try:
                ...
                if calendar_id != 'primary':
                    try:
                        logger.info(f"Event not found in specified calendar, trying primary")
                        event = self.service.events().get(calendarId='primary', eventId=event_id).execute()
                        # Update calendar_id for this operation
                        calendar_id = 'primary'
                    except Exception as inner_e:
                        logger.error(f"Event not found in primary calendar either: {inner_e}")
                        return None
                else:
            ...
            logger.info(f"Updated event {updated_event.get('id')} in calendar {calendar_id}")
